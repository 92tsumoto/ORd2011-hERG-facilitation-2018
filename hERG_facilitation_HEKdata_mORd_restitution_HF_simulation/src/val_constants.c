...
void val_consts(FILE *fp1)
{
	...
	// Late sodium current // (mS/uF).
	ina.tauhl = 200.0;				// (ms).
	ina.tauhl_CaMK = 3.0*ina.tauhl;	// 600 (ms).

	if(var.celltype==1){ // Endo.
		ina.Gna_late = 1.933*0.0075;
	} else if(var.celltype==2){ // Mid.
		ina.Gna_late = 1.933*0.0075;
	} else if(var.celltype==3){ // Epi.
		ina.Gna_late = 1.933*0.0075*0.6;
	}
	...
	if(var.celltype==1){ // Endo.
		...
		ito.Gto = 0.624*4.0*0.02;
	} else if(var.celltype==3){ // Epi.
		ito.Gto = 0.596*4.0*0.02;
	}
	...
	if(var.celltype == 1){ // Endo
		//ikr.Gkr = 0.046;
		ikr.Gkr = 0.727*var.gkr;	//(mS/uF)
	} else if(var.celltype == 2){ // Mid
		//ikr.Gkr = 0.8*0.046;
		ikr.Gkr = 1.0*0.8*var.gkr;
	} else if(var.celltype == 3){ // Epi
		//ikr.Gkr = 1.3*0.046;
		ikr.Gkr = 0.541*1.3*var.gkr;
	}

	// Slow delayed rectifier potassium current (Ikr) (mS/uF)
	if(var.celltype==1){ // Endo
		iks.Gks = 0.423*0.0034;
	} else if(var.celltype==2){ // Mid
		iks.Gks = 0.505*0.0034;
	} else if(var.celltype==3){ // Epi
		iks.Gks = 0.406*1.4*0.0034;
	}
	...
	if(var.celltype == 1){ // Endo
		...
		ik1.Gk1 = 0.473*1.3*0.1908;
	} else if(var.celltype == 3){ // Epi
		ik1.Gk1 = 0.447*1.2*0.1908;
	}
	...
	var.qna = 0.5224;
	var.qca = 0.1670;
	var.km_ca_act = 150E-6;		// (mM)
	if(var.celltype == 1){	// Endo
		var.Gnaca = 2.314*0.0008;// (uC/uF)
	} else if(var.celltype == 2){ // Mid 
		var.Gnaca = 2.314*1.4*0.0008;// (uC/uF)
	} else if(var.celltype == 3){ // Epi
		var.Gnaca = 2.314*1.1*0.0008;		
	}
	...
	if(var.celltype == 1){ // Endo
		...
		inak.G = 0.598*0.7*30.0;
	} else if(var.celltype == 3){ // Epi
		inak.G = 0.598*0.9*30.0;
	}
