...
void make_ExpTable()
{
	...
	// using Oocyte data
        //ikr.Txrss[vindex] = 1.0/(1.0+exp(-(v+23.8)/8.0));
        //ikr.Ttauxr[vindex] = 1.0/(0.00138*(v+14.2)/(1.0-exp(-0.123*(v+14.2)))+0.00061*(v+38.9)/(exp(0.145*(v+38.9))-1.0));
        //ikr.Trkr[vindex] = 1.0/(1.0+exp((v+9.0)/22.4));
        // for facilitation component for ikr 
        //ikr2.Txrss[vindex] = 1.0/(1.0+exp(-(v+23.8+26.6)/8.0));
        //ikr2.Ttauxr[vindex] = 1.0/(0.00138*(v+14.2)/(1.0-exp(-0.123*(v+14.2)))+0.00061*(v+38.9)/(exp(0.145*(v+38.9))-1.0));
        //ikr2.Trkr[vindex] = 1.0/(1.0+exp((v+9.0)/22.4));
        ...
        //inak.Tknao[vindex] = inak.ko_nao*exp(((1.0-inak.delta)*v*F)/(3.0*R*T)); // original ORd
        inak.Tknao[vindex] = inak.ko_nao*exp(((1.0+inak.delta)*v*F)/(3.0*R*T)); // correction by Kurata
		
		// ikb
        ikb.Txkb[vindex] = 1.0/(1.0+exp(-(v-14.48)/18.34));
		
		// icab
		icab.Texp[vindex] = exp(var.zca*v*F/R/T);

		// inab
		inab.Texp[vindex] = exp(var.zna*v*F/R/T);
	}
