...
void comp_ina(double x[])
{
	...
	ina.fast_phospho_CaMK = 1.0/(1.0 + CaMK.Km/CaMK.active);
	ina.fast = ina.Gna_fast*(x[0]-var.Ena)*x[1]*x[1]*x[1]*((1.0-ina.fast_phospho_CaMK)*ina.h*x[4]+ina.fast_phospho_CaMK*ina.hCaMK*x[6]);
	...
	ina.late_phospho_CaMK = 1.0/(1.0 + CaMK.Km/CaMK.active);
	ina.late = ina.Gna_late*(x[0]-var.Ena)*x[7]*((1.0-ina.late_phospho_CaMK)*x[8] + ina.late_phospho_CaMK*x[9]);
...
void comp_ito (double x[])
{
	...
	ito.phospho_CaMK = 1.0/(1.0 + CaMK.Km/CaMK.active);
	ito.ik = ito.Gto*(x[0]-var.Ek)*((1.0-ito.phospho_CaMK)*x[10]*ito.i + ito.phospho_CaMK*x[13]*ito.iCaMK);
...
// CaMK(CDI)
	...
	ical.tmp = 1.0+ical.kmn/x[38];
	ical.alpha_n = 1.0/((ical.kp2n/ical.km2n)+ical.tmp*ical.tmp*ical.tmp*ical.tmp);
	...
	ical.phi_ca = (var.zca*var.zca*F*F*x[0]/R/T)*(ical.gacai*x[38]*ical.exp_Ca-ical.gacao*var.cao)/(ical.exp_Ca-1.0);
	ical.phi_na = (var.zna*var.zna*F*F*x[0]/R/T)*(ical.ganai*x[34]*ical.exp_Na-ical.ganao*var.nao)/(ical.exp_Na-1.0);
	ical.phi_k = (var.zk*var.zk*F*F*x[0]/R/T)*(ical.gaki*x[36]*ical.exp_K-ical.gako*var.ko)/(ical.exp_K-1.0);
	...
	ical.phospho_CaMK = 1.0/(1.0 + CaMK.Km/CaMK.active);

	ical.ica = ical.ibarcal*x[16]*(1.0- ical.phospho_CaMK)*(ical.f*(1.0-x[24])+ical.fca*x[24]*x[21])
				+ ical.ibarcal_CaMK*x[16]*ical.phospho_CaMK*(ical.f_CaMK*(1.0-x[24])+ical.fca_CaMK*x[24]*x[21]);
	ical.icana = ical.ibarcana*x[16]*(1.0- ical.phospho_CaMK)*(ical.f*(1.0-x[24])+ical.fca*x[24]*x[21])
				+ ical.ibarcana_CaMK*x[16]*ical.phospho_CaMK*(ical.f_CaMK*(1.0-x[24])+ical.fca_CaMK*x[24]*x[21]);
	ical.icak = ical.ibarcak*x[16]*(1.0- ical.phospho_CaMK)*(ical.f*(1.0-x[24])+ical.fca*x[24]*x[21])
				+ ical.ibarcak_CaMK*x[16]*ical.phospho_CaMK*(ical.f_CaMK*(1.0-x[24])+ical.fca_CaMK*x[24]*x[21]);
...
void comp_ikr (double x[])
{
	...
	ikr.xr2 = ikr.Axr_fast2*x[41] + ikr.Axr_slow2*x[42];

	// Drug effects
	if(var.drug_type == 0){
		ikr.without = var.normal_block*ikr.Gkr*ikr.rategkr*ikr.xr*ikr.rkr*(x[0]-var.Ek);
		ikr.with = 0.0;
	} else if(var.drug_type == 1){
		ikr.without = var.normal_block*var.fraction_facil*ikr.Gkr*ikr.rategkr*ikr.xr*ikr.rkr*(x[0]-var.Ek);
		ikr.with = var.normal_block*(1.0-var.fraction_facil)*ikr.Gkr*ikr.rategkr*ikr.xr2*ikr.rkr2*(x[0]-var.Ek);
	//} else if(var.drug_type == 2){
	//	ikr.without = var.normal_block*var.fraction_facil*ikr.Gkr*ikr.rategkr*ikr.xr*ikr.rkr*(x[0]-var.Ek);
	//	ikr.with = var.normal_block*(1.0-var.fraction_facil)*ikr.Gkr*ikr.rategkr*ikr.xr2*ikr.rkr2*(x[0]-var.Ek);
	}
...
void comp_iks (double x[])
{
	...
	iks.ik = iks.Gks*(1.0+0.6/(1.0+pow((3.8E-5/x[37]),1.4)))*x[27]*x[28]*(x[0]-var.Eks);
...
void comp_inak (double x[])
{
	...
	inak.x1 = inak.a4*inak.a1*inak.a2+inak.b1*inak.b4*inak.b3+inak.a2*inak.b4*inak.b3+inak.b3*inak.a1*inak.a2; // correction by Kurata
...
void comp_icab (double x[])
{
	...
	icab.ca = icab.pcab*var.zca*var.zca*x[0]*F*F/R/T*(icab.gacai*x[37]*icab.exp-icab.gacao*var.cao)/(icab.exp-1.0);
...
void comp_inab (double x[])
{
	...
	inab.na = inab.pnab*var.zna*var.zna*x[0]*F*F/R/T*(x[33]*inab.exp-var.nao)/(inab.exp-1.0);
...
void comp_jrel (double x[])
{
	...
	if (jrel.tau_NP < 0.005){
		jrel.tau_NP = 0.005;
		//printf("jrel tauNP is small\n");
	}

	jrel.CaMKss = jrel.p*(-jrel.a_CaMK*ical.ica/(1.0+pow((1.5/x[40]),8)));
	jrel.tau_CaMK = jrel.b_tau_CaMK/(1.0+(0.0123/x[40]));
	if (jrel.tau_CaMK < 0.005){
		jrel.tau_CaMK = 0.005;
	}

	jrel.phospho_CaMK = 1.0/(1.0+CaMK.Km/CaMK.active);

	jrel.ca = (1.0-jrel.phospho_CaMK)*x[31]+jrel.phospho_CaMK*x[32];
...
void comp_jup (double x[])
{
	...
	jup.phospho_CaMK = 1.0/(1.0 + CaMK.Km/CaMK.active);

	jup.leak = 0.0039375*x[39]/15.0;

	jup.ca = (1.0-jup.phospho_CaMK)*jup.np + jup.phospho_CaMK*jup.CaMK - jup.leak;
